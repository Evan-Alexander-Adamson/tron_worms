...
window.addEventListener("load", () => {
    ...
    function drawBackgroundGrid(time) {
        ...
        const pulse = 0.3 + 0.2 * sin(time * 0.001); // Pulsing between 0.1 and 0.5
        const glow = 5 + 5 * sin(time * 0.001); // Pulsing shadowBlur
        ctx.save();
        ctx.strokeStyle = `rgba(255, 0, 255, ${pulse})`; // Neon purple with pulsing alpha
        ctx.lineWidth = 1;
        ctx.shadowColor = 'rgba(255, 0, 255, 1)'; // Solid neon purple for shadow
        ctx.shadowBlur = glow;
        ctx.beginPath();
    ...
    class Portal {
        ...
        draw(time) {
            const pulse = 0.5 + 0.5 * sin(time * 0.005); // Pulsing effect
            ctx.save();
            ctx.beginPath();
            ctx.arc(this.position.x, this.position.y, this.radius, 0, TWO_PI);
            ctx.strokeStyle = `hsla(300, 100%, 70%, ${pulse})`; // Portal color with pulsing alpha
            ctx.lineWidth = 5;
            ctx.shadowColor = `hsla(300, 100%, 70%, ${pulse})`;
            ctx.shadowBlur = 15;
            ctx.stroke();
            ctx.restore();
        }
    ...
    function animate() {
        ...
        drawBackgroundGrid(currentTime);
        ...
        // Draw portals
        portals.forEach(portal => portal.draw(currentTime));
